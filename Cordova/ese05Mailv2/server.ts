...
function writeCookie(token, res){

    //httpOnly = non sia accessibile da js ma solo da html
    //secure = abilita l'utilizz del token solo alla pagina con https
    //samesite = il token viene reso visibile anche per le richieste provenienti da pagine non inviate dal server

    let cookie=`token = ${token}; 
                Max-age = ${DURATA_TOKEN}; 
                path = /;
                HttpOnly = true;
                Secure = true;
                SameSite = false;`;
